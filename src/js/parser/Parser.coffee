define [
	...
	parseColorComponents = (tokens, errors) ->
		for error in errors
			tokens.expect IDENTIFIER, error
			tokens.past()
	...
	parseColor = do ->
		...
		alphaError = 'Expected alpha value'

		rgbaErrors = [rgbErrors..., alphaError]

		hslErrors = [
			'Expected hue value'
			'Expected saturation value'
			'Expected lightness value'
		]

		hslaErrors = [hslErrors..., alphaError]

		(tokens) ->
			if tokens.match 'rgb'
				tokens.adv()
				[red, green, blue] = parseColorComponents tokens, rgbErrors
				{ format: 'rgb', red, green, blue }
			else if tokens.match 'rgba'
				tokens.adv()
				[red, green, blue, alpha] = parseColorComponents tokens, rgbaErrors
				{ format: 'rgba', red, green, blue, alpha }
			else if tokens.match 'hsl'
				tokens.adv()
				[hue, saturation, lightness] = parseColorComponents tokens, hslErrors
				{ format: 'hsl', hue, saturation, lightness }
			else if tokens.match 'hsla'
				tokens.adv()
				[hue, saturation, lightness, alpha] = parseColorComponents tokens, hslaErrors
				{ format: 'hsla', hue, saturation, lightness, alpha }
			else
				throw 'Expected one of rgb, rgba, hsl or hsla'

	Parser.parseColor = parseColor # for debugging/testing only
	...
	parseColors = (tokens) ->
		...
		until tokens.match 'PLAYER'
			...
			colors.push Object.assign { name }, parseColor tokens
