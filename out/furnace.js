(function () {
  ...
  else (function () {// Generated by CoffeeScript 2.3.1
(function() {
...
(function() {
  define('Font',[], function() {
    var colorBindings, convert, namedSprites, verify;
    colorBindings = [
      {
        name: '1',
        red: '255',
    ...
    verify = (function() {
      var LINE_COUNT;
      LINE_COUNT = 16;
      return function(namedSprites) {
        namedSprites.forEach(function(arg) {
          var data, lineCount, name;
          data = arg.data, name = arg.name;
...
(function() {
  define('generator/SpriteSheetGenerator',['Util', 'SpriteSheet'], function(Util, SpriteSheet) {
    ...
    SpriteSheetGenerator.generate = function(stringedSprites, colorBindings, scale) {
      var canvas, colorsByBinding, con2d, namedSprites, paint, pixel, spriteSheet, width;
      ...
      colorsByBinding = Util.arrayToObject(colorBindings.map(function(binding) {
        var stringedColor;
        stringedColor = (function() {
          switch (binding.format) {
            case 'rgb':
              return `rgb(${binding.red},${binding.green},${binding.blue})`;
            case 'rgba':
              return `rgba(${binding.red},${binding.green},${binding.blue},${binding.alpha})`;
            case 'hsl':
              return `hsl(${binding.hue},${binding.saturation}%,${binding.lightness}%)`;
            case 'hsla':
              return `hsla(${binding.hue},${binding.saturation}%,${binding.lightness}%,${binding.alpha})`;
          }
        })();
        return {
          name: binding.name,
...
// Generated by CoffeeScript 2.3.1
(function() {
  define('TextBubble',['Text', 'Vec2', 'con2d'], function(Text, Vec2, con2d) {
    'use strict';
    var TextBubble;
    TextBubble = function() {
    ...
    TextBubble.prototype.draw = function() {
      ...
      this.lines.forEach((line, index) => {
        Text.drawAt(line, this.position.x + 4, this.position.y + index * 18 + 4);
      });
    };
    TextBubble.prototype.setText = function(text) {
      ...
      this.length = Math.max(...this.lines.map(function({length}) {
        return length;
      }));
      boxWidth = this.length * 16 + 8;
...
(function() {
  define('sound/SoundUtil',[], function() {
    ...
    return {range, options, randomParameters, encode, decode, applyRamp, getEnvelope};
  });
...
(function() {
  define('sound/generations/g0/s0',['sound/SoundUtil'], function(SoundUtil) {
    ...
    return {generation, type, spec, build};
  });
...
(function() {
  define('sound/generations/g0/s4',['sound/SoundUtil'], function(SoundUtil) {
    ...
    build = function(context, parameters) {
      var b0, b1, b2, b3, b4, b5, b6, bufferSize, gain, i, j, noiseBuffer, output, ref, start, white, whiteNoise;
...
(function() {
  define('sound/Sound',['sound/SoundUtil', 'sound/generations/g0/s0', 'sound/generations/g0/s1', 'sound/generations/g0/s2', 'sound/generations/g0/s3', 'sound/generations/g0/s4'], function(SoundUtil) {
    'use strict';
    var CURRENT_GENERATION, audioContext, generate, getRandomParameters, getRecorder, getSpec, i, j, key, play, ref, synth, synthForKey, synthKey, synths;
    ...
    return {CURRENT_GENERATION, getSpec, getRandomParameters, generate, play};
  });
  ...
  define('generator/Generator',['generator/SpriteSheetGenerator', 'Util', 'World', 'Item', 'Items', 'Set', 'rule/NearRule', 'rule/LeaveRule', 'rule/EnterRule', 'rule/UseRule', 'rule/RuleSet', 'Vec2', 'Level', 'sound/Sound', 'sound/SoundUtil'], function(SpriteSheetGenerator, Util, World, Item, Items, Set, NearRule, LeaveRule, EnterRule, UseRule, RuleSet, Vec2, Level, Sound, SoundUtil) {
    ...
    generateSounds = function(sounds) {
      ...
      promises = sounds.map(function(entry) {
        var generation, parameters, spec, type;
...
(function() {
  define('Game',['SystemBus', 'World', 'generator/Generator'], function(SystemBus, World, Generator) {
    ...
    Game.prototype.init = function(tree) {
      return Generator.generate(tree).then((world) => {
        this.world = world;
    ...
    Game.prototype.start = function() {
      ...
      frame = () => {
        this.world.update();
        this.world.draw();
        return this.requestId = window.requestAnimationFrame(frame);
      };
      return frame();
    };
...
// Generated by CoffeeScript 2.3.1
(function() {
  define('tokenizer/IterableString',['tokenizer/TokenCoords'], function(TokenCoords) {
    'use strict';
    var IterableString;
    IterableString = function(str) {
      this.str = str;
      this.pointer = 0;
      this.marker = 0;
      this.line = 1;
...
(function() {
  define('tokenizer/TokIdentifier',['tokenizer/Token'], function(Token) {
    'use strict';
    var TokIdentifier;
    TokIdentifier = function(value, coords) {
      this.value = value;
      this.coords = coords;
    };
...
(function() {
  define('tokenizer/TokKeyword',['tokenizer/Token'], function(Token) {
    'use strict';
    var TokKeyword;
    TokKeyword = function(value, coords) {
      this.value = value;
      this.coords = coords;
    };
...
(function() {
  define('tokenizer/TokCommSL',["tokenizer/Token"], function(Token) {
    'use strict';
    var TokCommSL;
    TokCommSL = function(value, coords) {
      this.value = value;
      this.coords = coords;
    };
    TokCommSL.prototype = Object.create(Token.prototype);
    TokCommSL.prototype.constructor = TokCommSL;
...
(function() {
  define('tokenizer/TokCommML',['tokenizer/Token'], function(Token) {
    'use strict';
    var TokCommML;
    TokCommML = function(value, coords) {
      this.value = value;
      this.coords = coords;
    };
    TokCommML.prototype = Object.create(Token.prototype);
    TokCommML.prototype.constructor = TokCommML;
...
(function() {
  define('tokenizer/TokComma',['tokenizer/Token'], function(Token) {
    'use strict';
    var TokComma;
    TokComma = function(coords) {
      this.coords = coords;
    };
    TokComma.prototype = Object.create(Token.prototype);
    TokComma.prototype.constructor = TokComma;
...
(function() {
  define('tokenizer/TokSemicolon',['tokenizer/Token'], function(Token) {
    'use strict';
    var TokSemicolon;
    TokSemicolon = function(coords) {
      this.coords = coords;
    };
    TokSemicolon.prototype = Object.create(Token.prototype);
    TokSemicolon.prototype.constructor = TokSemicolon;
...
(function() {
  define('tokenizer/TokAssignment',['tokenizer/Token'], function(Token) {
    'use strict';
    var TokAssignment;
    TokAssignment = function(coords) {
      this.coords = coords;
    };
    TokAssignment.prototype = Object.create(Token.prototype);
    TokAssignment.prototype.constructor = TokAssignment;
...
(function() {
  define('tokenizer/TokNewLine',['tokenizer/Token'], function(Token) {
    'use strict';
    var TokNewLine;
    TokNewLine = function(coords) {
      this.coords = coords;
    };
    TokNewLine.prototype = Object.create(Token.prototype);
    TokNewLine.prototype.constructor = TokNewLine;
...
(function() {
  define('tokenizer/Tokenizer',['tokenizer/IterableString', 'tokenizer/TokEnd', 'tokenizer/TokIdentifier', 'tokenizer/TokStr', 'tokenizer/TokKeyword', 'tokenizer/TokCommSL', 'tokenizer/TokCommML', 'tokenizer/TokComma', 'tokenizer/TokSemicolon', 'tokenizer/TokArrow', 'tokenizer/TokAssignment', 'tokenizer/TokNewLine'], function(IterableString, TokEnd, TokIdentifier, TokStr, TokKeyword, TokCommSL, TokCommML, TokComma, TokSemicolon, TokArrow, TokAssignment, TokNewLine) {
    ...
    keywords = ['PARAM', 'COLORS', 'PLAYER', 'OBJECTS', 'SETS', 'SOUNDS', 'NEARRULES', 'LEAVERULES', 'ENTERRULES', 'USERULES', 'LEGEND', 'LEVELS', 'rgb', 'rgba', 'hsl', 'hsla', 'blocking', 'or', 'and', 'minus', 'consume', 'give', 'heal', 'hurt', 'teleport', 'message', 'checkpoint', 'sound'];
    return Tokenizer;
  });
  ...
  define('parser/TokenList',['ParserError'], function(ParserError) {
    ...
    TokenList.prototype.expect = function(token, exMessage) {
      if (this.match(token)) {
        ...
        throw new ParserError(exMessage, this.cur().coords.line, this.cur().coords.col);
      }
    };
...
(function() {
  define('parser/Parser',['tokenizer/TokEnd', 'tokenizer/TokIdentifier', 'tokenizer/TokStr', 'tokenizer/TokKeyword', 'tokenizer/TokCommSL', 'tokenizer/TokCommML', 'tokenizer/TokComma', 'tokenizer/TokSemicolon', 'tokenizer/TokArrow', 'tokenizer/TokNewLine', 'tokenizer/TokAssignment', 'tokenizer/TokenCoords', 'parser/TokenList'], function(TokEnd, TokIdentifier, TokStr, TokKeyword, TokCommSL, TokCommML, TokComma, TokSemicolon, TokArrow, TokNewLine, TokAssignment, TokenCoords, TokenList) {
    'use strict';
    var ARROW, ASSIGNMENT, COMMA, END, IDENTIFIER, NEWLINE, Parser, SEMICOLON, STR, chompNL, parse, parseColor, parseColorComponents, parseColors, parseEnterRules, parseLeaveRules, parseLegend, parseLevels, parseNearRules, parseObjects, parseParams, parsePlayer, parseSets, parseSounds, parseUseRules;
    ...
    parseColorComponents = function(tokens, errors) {
      var error, i, len, results;
      results = [];
      for (i = 0, len = errors.length; i < len; i++) {
        error = errors[i];
        tokens.expect(IDENTIFIER, error);
        results.push(tokens.past());
      }
      return results;
    };
    parseColor = (function() {
      var alphaError, hslErrors, hslaErrors, rgbErrors, rgbaErrors;
      rgbErrors = ['Expected red value', 'Expected green value', 'Expected blue value'];
      alphaError = 'Expected alpha value';
      rgbaErrors = [...rgbErrors, alphaError];
      hslErrors = ['Expected hue value', 'Expected saturation value', 'Expected lightness value'];
      hslaErrors = [...hslErrors, alphaError];
      return function(tokens) {
        var alpha, blue, green, hue, lightness, red, saturation;
        if (tokens.match('rgb')) {
          tokens.adv();
...
(function() {
  define('validator/Validator',['validator/ValidatorError', 'Util'], function(ValidatorError, Util) {
    ...
    validateParam = function(paramSpec, levelsSpec) {
      var levelsSet, paramsSet, ref, ref1, startLevelName;
      ...
      if (paramsSet.start_location != null) {
        ...
        if (!((0 <= (ref = +paramsSet.start_location.parts[0].value) && ref <= levelsSet[startLevelName].data[0].value.length))) {
          throw new ValidatorError(paramsSet.start_location.parts[0], 'Start location coordinates must be within level bounds');
        }
        if (!((0 <= (ref1 = +paramsSet.start_location.parts[1].value) && ref1 <= levelsSet[startLevelName].data.length))) {
    ...
    validateColors = function(colorsSpec) {
      ...
      colorsSpec.forEach(function(color) {
        ...
        if ((ref = color.format) === 'rgb' || ref === 'rgba') {
    ...
    validateTeleport = function(teleportSpec, levelsSet) {
      ...
      if (!((0 <= (ref = +teleportSpec.x.value) && ref <= level.data[0].value.length))) {
        throw new ValidatorError(teleportSpec.x, "Teleport coordinates must be within level bounds");
      }
      if (!((0 <= (ref1 = +teleportSpec.y.value) && ref1 <= level.data.length))) {
...
// Generated by CoffeeScript 2.3.1
(function() {
  define('compilers/js/main',['Util'], function(Util) {
    'use strict';
    var compileCode, compileData, compileEnterRule, compileEnterRules, compileLeaveRule, compileLeaveRules, compileLevel, compileLevelSize, compileLevels, compileNearRule, compileNearRules, compileObject, compileObjects, compilePlayerSprite, compilePlayerSprites, compileSet, compileSets, compileSpriteData, compileUseRule, compileUseRules, isSetReference, makeList, resolveItem;
    compileSpriteData = function(data, colors) {
      var bytes, color, colorData, colorName, line;
      colorData = (function() {
    ...
    compilePlayerSprite = function({name, data}, colors) {
      return `'${name}': {\n	data: ${compileSpriteData(data, colors)},\n}`;
    };
    compilePlayerSprites = function(playerSprites, colors) {
      var lines, push;
      lines = [];
    ...
    compileObject = function({name, data, blocking}, colors) {
      return `'${name}': {\n	blocking: ${Boolean(blocking)},\n	data: ${compileSpriteData(data, colors)},\n}`;
    };
    compileObjects = function(objects, colors) {
      var lines, push;
      lines = [];
      push = lines.push.bind(lines);
      push("const objects = {");
    ...
    compileLevelSize = function(levels) {
      return `{${(levels.map(function({name, data}) {
        return `'${name}' : {\n	x: ${data[0].length},\n	y: ${data.length},\n}`;
      })).join(',\n')}}`;
    };
    compileLevel = function(legend, {name, data}) {
      var lines;
      lines = data.map(function(line) {
        return Array.prototype.map.call(line, function(item) {
          return legend[item];
        });
      });
      return `'${name}': ${JSON.stringify(lines)}`;
    };
    compileLevels = function(legend, levels) {
      var legendIndexed, lines, push;
    ...
    compileSet = function(setSpec, setDefinitions) {
      var elements, operand1, operand2, set;
      if (setSpec.elements != null) {
        ...
        switch (setSpec.operator) {
          case 'or':
            operand1.forEach(function(element) {
              set.add(element);
            });
            operand2.forEach(function(element) {
              set.add(element);
            });
            break;
          case 'and':
      ...
      elements = (Array.from(set)).map(function(element) {
        return `'${element}'`;
      });
      return `'${setSpec.name}': new Set([${elements.join(', ')}])`;
    };
    compileSets = function(sets) {
      var lines, push, setDefinitions;
      lines = [];
      push = lines.push.bind(lines);
      push("const sets = {");
      setDefinitions = {};
    ...
    resolveItem = function(reference) {
      switch (reference) {
        case '_terrain':
          return 'terrainItem';
        case '_inventory':
    ...
    compileNearRule = function(rule) {
      var join, push, terrainItemCondition;
      ({push, join} = makeList());
      terrainItemCondition = isSetReference(rule.inTerrainItemName) ? `sets['${rule.inTerrainItemName}'].has(terrainItem)` : `terrainItem === '${rule.inTerrainItemName}'`;
      push(`if (${terrainItemCondition}) {`);
      if (rule.outTerrainItemName != null) {
        push(`setTerrainItem(neighbor, ${resolveItem(rule.outTerrainItemName)})`);
      }
      if ((rule.healthDelta != null) && rule.healthDelta !== 0) {
        push(`state.player.health += ${rule.healthDelta}`);
      }
      push("}");
      return join('\n');
    };
    compileNearRules = function(rules) {
      var join, push, ruleLines;
      ({push, join} = makeList());
    ...
    compileLeaveRule = function(rule) {
      var join, push, terrainItemCondition;
      ({push, join} = makeList());
      terrainItemCondition = isSetReference(rule.inTerrainItemName) ? `sets['${rule.inTerrainItemName}'].has(terrainItem)` : `terrainItem === '${rule.inTerrainItemName}'`;
      push(`if (${terrainItemCondition}) {`);
    ...
    compileLeaveRules = function(rules) {
      var join, push, ruleLines;
      ({push, join} = makeList());
    ...
    compileEnterRule = function(rule) {
      var join, push, terrainItemCondition;
      ({push, join} = makeList());
      terrainItemCondition = isSetReference(rule.inTerrainItemName) ? `sets['${rule.inTerrainItemName}'].has(terrainItem)` : `terrainItem === '${rule.inTerrainItemName}'`;
      push(`if (${terrainItemCondition}) {`);
      ...
      if (rule.give != null) {
        rule.give.forEach(function({itemName, quantity}) {
    ...
    compileEnterRules = function(rules) {
      var join, push, ruleLines;
      ({push, join} = makeList());
    ...
    compileUseRule = function(rule) {
      var inventoryItemCondition, join, push, terrainItemCondition;
      ({push, join} = makeList());
      terrainItemCondition = isSetReference(rule.inTerrainItemName) ? `sets['${rule.inTerrainItemName}'].has(terrainItem)` : `terrainItem === '${rule.inTerrainItemName}'`;
      inventoryItemCondition = isSetReference(rule.inInventoryItemName) ? `sets['${rule.inInventoryItemName}'].has(inventoryItem.name)` : `inventoryItem.name === '${rule.inInventoryItemName}'`;
    ...
    compileUseRules = function(rules) {
      var join, push, ruleLines;
      ({push, join} = makeList());
    ...
    compileData = function(spec) {
      var colorsIndexed, params;
      params = Util.arrayToObject(spec.params, 'name', 'parts');
      colorsIndexed = new Map;
      spec.colors.forEach(function(color) {
    ...
    return {compileData, compileCode};
  });
...
// Generated by CoffeeScript 2.3.1
(function() {
  define('main',['SystemBus', 'Text', 'Game', 'tokenizer/Tokenizer', 'parser/Parser', 'validator/Validator', 'extractor/ValueExtractor', 'KeyListener', 'import-export/FromPng', 'AjaxUtil', 'compilers/js/main'], function(SystemBus, Text, Game, Tokenizer, Parser, Validator, ValueExtractor, KeyListener, FromPng, AjaxUtil, JsCompiler) {
    'use strict';
    ...
    editorEventHandlers = {
      'compile': function(data) {
        ...
        compile(tree);
        if (localStorage['js-compiler'] != null) {
...
(function() {
  define('editor/furnace-rules',[],function() {
    ...
    ace.define('ace/mode/furnace-rules', function(require_, exports, module) {
      var FurnaceHighlightRules, TextHighlightRules, oop;
      oop = require_('../lib/oop');
      TextHighlightRules = require_('./text_highlight_rules').TextHighlightRules;
      FurnaceHighlightRules = function() {
        var BINDING, COMMENT, PLAIN, RESERVED, SECTION, SETBINDING, STRING;
        SECTION = 'keyword';
        BINDING = 'variable';
